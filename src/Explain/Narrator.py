...
def main():
    ...
    def one_entailment(entailments:list):
        one_ent_out_df = pd.DataFrame(columns=['entailment', 'correlation', 'p-value'])
        for ents_dict in entailments:
            for key in tqdm(ents_dict.keys()):
                for entailment in ents_dict[key]:
                    ...
                    if len(set(co_ex_v)) == 1:
                        co_ex_v[0] += 0.01
                        co_ex_v[1] -= 0.01

                    corr, p_val = stats.pearsonr(co_ex_v, auc_v)
                    if p_val <= 0.05:
                        plausible = pd.DataFrame({'entailment': entailment, 'correlation':corr, 'p-value': p_val}, index=[1])
                        one_ent_out_df = one_ent_out_df.append(plausible, ignore_index=True)
                            
                        
        one_ent_out_df.to_csv(f"{correlation_dir}/OneEntailment.csv", index=False)

    def two_entailments(entailments:list):

        two_ents_out_df = pd.DataFrame(columns=['entailment1', 'entailment2', 'correlation', 'p-value'])
        for ents_dict in entailments:
            for key in tqdm(ents_dict.keys()):
                for entailment1 in ents_dict[key]:
                    for entailment2 in ents_dict[key]:
                        if entailment1 == entailment2:
                            continue 
                    auc_v = []
                    co_ex_v = []
                    f = open(os.path.join(correlation_dir, 'tra.csv'), 'r')
                    ...
                    for row in reader:
                        ...
                        F_id = row['S-T']
                        [source_domain, target_domain] = F_id.split('-')
                        co_ex1 = co_existence_quaternary(entailment1, source_domain, target_domain)
                        co_ex2 = co_existence_quaternary(entailment2, source_domain, target_domain)
                        if co_ex1 == 0 and co_ex2 == 0:
                            co_ex_v.append(0)
                            auc_v.append(auc)
                    ...
                    if len(set(co_ex_v)) == 1 and len(co_ex_v) > 1:
                        co_ex_v[0] += 0.01
                        co_ex_v[1] -= 0.01

                    if 1 < len(co_ex_v) == len(auc_v):
                        corr, p_val = stats.pearsonr(co_ex_v, auc_v)
                        if abs(corr) >= 0.1 and p_val <= 0.05:
                            plausible = pd.DataFrame({'entailment1': entailment1, 'entailment2':entailment2, 'correlation':corr, 'p-value': p_val}, index=[1])
                            two_ents_out_df = two_ents_out_df.append(plausible, ignore_index=True)
        
        two_ents_out_df.to_csv(f"{correlation_dir}/TwoEntailments.csv", index=False)

    def three_entailments(entailments:list):

        three_ents_out_df = pd.DataFrame(columns=['entailment1', 'entailment2', 'entailment3', 'correlation', 'p-value'])
        f_2ent = open(os.path.join(correlation_dir, 'TwoEntailments.csv'), 'r')
        reader_2ent = csv.DictReader(f_2ent)
        for ents_dict in entailments:
            for key in tqdm(ents_dict.keys()):
                for entailment3 in ents_dict[key]:
                    for r in reader_2ent:
                        entailment1, entailment2, coe, p_val = r['entailment1'], r['entailment2'], float(r['correlation']), float(r['p-value'])
                        if abs(coe) >= 0.12 and p_val < 0.05:
                
                    
                            if entailment1 == entailment3 or entailment2 == entailment3:
                                continue
                            ...
                            for row in reader:
                                ...
                                co_ex1 = co_existence_quaternary(entailment1, source_domain, target_domain)
                                co_ex2 = co_existence_quaternary(entailment2, source_domain, target_domain)
                                co_ex3 = co_existence_quaternary(entailment3, source_domain, target_domain)
                                if co_ex1 == 0 and co_ex2 == 0 and co_ex3 == 0:
                                    co_ex_v.append(0)
                                    auc_v.append(auc)
                            ...
                            if 1 < len(co_ex_v) == len(auc_v):
                                corr, p_val = stats.pearsonr(co_ex_v, auc_v)
                                if abs(corr) >= 0.1 and p_val <= 0.05:
                                    plausible = pd.DataFrame({'entailment1': entailment1, 'entailment2':entailment2, 'entailment3':entailment3, 'correlation':corr, 'p-value': p_val}, index=[1])
                                    three_ents_out_df = three_ents_out_df.append(plausible, ignore_index=True)
    
        three_ents_out_df.to_csv(f"{correlation_dir}/ThreeEntailments.csv", index=False)


    # get_indices()

    #one_entailment(entailments)

    #two_entailments(entailments)

    three_entailments(entailments)
